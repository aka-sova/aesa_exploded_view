...
export const tutorialEn = {
  ...
  'tut.doppler.title': 'Doppler: the other half of the measurement',
  'tut.doppler.body': `Every echo comes back shifted in frequency by 2·v<sub>r</sub>/λ — at X-band about 63 Hz for each metre per second of closing speed. The map under the aperture inset is what the processor sees from the current dwell: <b>range</b> up the vertical axis, <b>velocity</b> across, the bright ridge at zero velocity is <b>ground clutter</b> (only when the beam points low), and each target is a blip.<br><br>With a low PRF the radar samples Doppler too slowly: velocities fold into ±PRF·λ/4 (±9.5 m/s at 1200 Hz), so a 235 m/s jet shows up at some small folded speed, and any target whose speed is a multiple of PRF·λ/2 — every 19 m/s — lands on the clutter ridge: a <b>blind speed</b>. The <b>MTI</b> filter cancels the ridge but also notches every blind speed. Raising the PRF widens the velocity window but folds <b>range</b> instead, as Rᵤ shrinks — the low/high-PRF trade every pulse-Doppler radar lives with.`,
  'tut.doppler.try': 'Switch MTI on and watch the ridge — and T2, which sits at a blind speed — vanish; move PRF and watch T2 reappear while the range axis folds. Click a row in the TRACKS table to pin the map to that target\'s latest dwell.',
...
export const tutorialRu = {
  ...
  'tut.doppler.title': 'Доплер: вторая половина измерения',
  'tut.doppler.body': `Каждое эхо возвращается со сдвигом частоты 2·v<sub>r</sub>/λ — в X-диапазоне около 63 Гц на каждый метр в секунду скорости сближения. Карта под врезкой апертуры — то, что видит процессор в текущем дискрете: <b>дальность</b> по вертикали, <b>скорость</b> по горизонтали, яркий гребень на нулевой скорости — <b>отражения от земли</b> (только когда луч смотрит низко), а каждая цель — отметка.<br><br>При низкой ЧПИ доплер дискретизируется слишком редко: скорости сворачиваются в ±ЧПИ·λ/4 (±9,5 м/с при 1200 Гц), поэтому самолёт на 235 м/с виден на какой-то малой свёрнутой скорости, а цель, чья скорость кратна ЧПИ·λ/2 — каждые 19 м/с, — попадает на гребень земли: <b>слепая скорость</b>. Фильтр <b>СДЦ</b> подавляет гребень, но вырезает и каждую слепую скорость. Повышение ЧПИ расширяет окно скоростей, но зато сворачивает <b>дальность</b> — Rодн уменьшается. Это компромисс низкой/высокой ЧПИ, с которым живёт каждая импульсно-доплеровская РЛС.`,
  'tut.doppler.try': 'Включите СДЦ и наблюдайте, как исчезают гребень и T2 (она на слепой скорости); изменяйте ЧПИ — T2 появится снова, а ось дальности свернётся. Нажмите на строку в таблице ТРАССЫ, чтобы закрепить карту за последним дискретом по этой цели.',
